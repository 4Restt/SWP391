...
<html lang="en">
    ...
    <section class="py-5">
        <div class="container">
            <div class="d-flex justify-content-between align-items-center">
                <h1 class="page-title pb-2">My Order</h1>
    ...
    <section class="contact-us-wrap py-2">
        <div class="container">
            <div class="row">
                <div class="col-12 mt-2">
                    <div class="">
                        <table id="userTable" class="table table-striped table-bordered table-sm">
                            <thead>
                                <tr>
                                    <th class="border-bottom p-2" style="min-width: 50px;">Order Id</th>

                                    <th class="border-bottom p-2" style="min-width: 150px;">Date</th>
                                    <th class="border-bottom p-2" style="min-width: 150px;">Total Price</th>
                                    <th class="border-bottom p-2" style="min-width: 100px;">Status</th>
                                    <th class="border-bottom p-2" style="min-width: 150px;"></th>
                                </tr>
                            </thead>
                            <tbody>
                                <c:forEach items="${listOrder}" var="lo" varStatus="loop">
                                    <tr>
                                        <th class="p-2">${lo.getId()}</th>

                                        <td class="p-2">${lo.getDate()}</td>
                                        <td class="p-2">${lo.getTotalprice()}</td>
                                        <td class="p-2">
                                            <c:choose>
                                                ...
                                                <c:when test="${lo.getStatus() eq 4}">
                                                    Shipping
                                                </c:when>
                                                <c:when test="${lo.getStatus() eq 5}">
                                                    Success
                                                </c:when>
                                                <c:when test="${lo.getStatus() eq 6}">
                                                    Cancel
                                                </c:when>
                                            </c:choose>
                                        </td>
    ...
    <c:forEach items="${listOrder}" var="lo" varStatus="loop">
                                    ...
                                    <div class="col-lg-10 col-xl-8">
                                        <div class="card" style="border-radius: 10px;">
                                            ...
                                            <div class="card-body p-4">
                                                ...
                                                <div class="d-flex justify-content-between align-items-center mb-4">
                                                    <p class="lead fw-normal mb-0" style="color: #a8729a;">Track Order</p>
                                                    <p class="small text-muted mb-0"></p>
                                                </div>
                                                <div class="card">
                                                    <div class="card-body">
                                                        ...
                                                        <div class="row d-flex align-items-center">
                                                            ...
                                                            <div class="col-md-12">
                                                                <!-- HTML -->
                                                                <div class="progress" id="progress-bar" style="height: 6px; border-radius: 16px;">
                                                                    <div class="progress-bar" id="progress-bar-inner-${lo.getId()}" role="progressbar" style="width: 0%; border-radius: 16px; background-color: #a8729a;" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100"></div>
                                                                </div>
                                                                <div class="d-flex align-items-start mb-1">
                                                                    <p class="text-muted mt-1 mb-0 small ms-xl-5" id="status-${lo.getId()}-0">Processing</p>
                                                                    <p class="text-muted mt-1 mb-0 small ms-xl-5" id="status-${lo.getId()}-1">Accept</p>
                                                                    <p class="text-muted mt-1 mb-0 small ms-xl-5" id="status-${lo.getId()}-2">Prepare</p>
                                                                    <p class="text-muted mt-1 mb-0 small ms-xl-5" id="status-${lo.getId()}-3">Delivered</p>
                                                                    <p class="text-muted mt-1 mb-0 small ms-xl-5" id="status-${lo.getId()}-4">Success</p>
                                                                </div>
                                                ...
                                                <div class="d-flex justify-content-between pt-2">
                                                    ...
                                                    <div>
                                                        <p class="text-muted mb-0"><span class="fw-bold me-2">Total:</span> $${lo.getTotalprice()}</p>
                                                    </div>
                                                </div>

                                                <div class="d-flex justify-content-between pt-2">
                                                    <div>
                                                        <p class="text-muted mb-0"><span class="fw-bold me-2">Order Id:</span> ${lo.getId()}</p>
                                                    </div>
                                                </div>

                                                <div class="d-flex justify-content-between pt-2">
                                                    <div>
                                                        <p class="text-muted mb-0"><span class="fw-bold me-2">Order Date:</span> ${lo.getDate()}</p>
                                                    </div>
                                                </div>

                                                <div class="d-flex justify-content-between mb-4">
                                                    <div>
                                                        <p class="text-muted mb-0"><span class="fw-bold me-2">Address:</span> ${lo.getCustomerAddress()}</p>
                                                    </div>
                                                    <div>
                                                        <p class="text-muted mb-0"><span class="fw-bold me-2">Delivery Charges:</span> Free</p>
                                                    </div>
                                                </div>
        ...
        <script>
// JavaScript
// Calculate the width of the progress bar based on the number of statuses and current status
            function updateProgressBar_${lo.getId()}(currentStatus, totalStatuses) {
                var progress = ((currentStatus + 1) / totalStatuses) * 100;
                document.getElementById('progress-bar-inner-${lo.getId()}').style.width = progress + '%';

// Update status texts to show the current status
                for (var i = 0; i <= totalStatuses; i++) {
                    var statusElement = document.getElementById('status-${lo.getId()}-' + i);
                    if (i <= currentStatus) {
                        statusElement.style.fontWeight = 'bold'; // Optionally, highlight completed statuses
                    } else {
...
// Call this function with appropriate arguments when you want to update the progress
            var currentStatus_${lo.getId()} = ${lo.getStatus()}; // Example current status
            var totalStatuses_${lo.getId()} = 6; // Example total number of statuses
            updateProgressBar_${lo.getId()}(currentStatus_${lo.getId()}, totalStatuses_${lo.getId()});
        </script>
        <!-- View Appintment End -->
                ...
                <div class="modal-content">
                    <div class="modal-body py-5">
                        <div class="text-center">
                            ...
                            <div class="mt-4">
                                <h4>Cancel Order</h4>
                                <p class="para-desc mx-auto text-muted mb-0">This step will Cancel the order</p>
                                <p class="para-desc mx-auto text-muted mb-0">Please confirm </p>
                                <div class="mt-4">
                                    <a href="ordercancle?id=${lo.getId()}&status=0" class="btn btn-soft-danger">Cancel</a>
                                </div>
                            </div>
                        </div>
                    </div>
